...
function gavias_inusti_form_system_theme_settings_alter(&$form, &$form_state) {
  ...
  if(count($languagesAll) > 1){
    foreach ($languagesAll as $key => $language) {
      $form['header']['link_request' . $language->getID()] = array(
        '#type' => 'textfield',
        '#title' => t('Listing Search Action ' . $language->getID() ),
        '#default_value' => theme_get_setting('link_request'. $language->getID()) ? theme_get_setting('link_request'. $language->getID()) : ''
      ); 
    }
  }
